/*
  ...
  3. Genel Şartlar
    - Her standart altında görseldeki genel şartlar eklenir
*/

-- Kontrol Ortamı Bileşeni
INSERT INTO ic_components (code, name, description, order_index, color)
VALUES 
  ('KOS', 'KONTROL ORTAMI', 'İdarelerin hedeflerine ulaşabilmesi için gerekli ortam ve şartları sağlayan temel unsurlar bütünüdür.', 1, '#3B82F6');

-- Standartları ekle
WITH component AS (
  SELECT id FROM ic_components WHERE code = 'KOS'
)
...
WITH standard AS (
  SELECT id FROM ic_standards WHERE code = 'KOS.1'
)
INSERT INTO ic_general_conditions (standard_id, code, condition_text, order_index)
SELECT 
  standard.id,
  'KOS 1.1',
...
WITH standard AS (
  SELECT id FROM ic_standards WHERE code = 'KOS.2'
)
INSERT INTO ic_general_conditions (standard_id, code, condition_text, order_index)
SELECT 
  standard.id,
...
SELECT 
  standard.id,
  'KOS 2.4',
  'İdarenin ve birimlerin teşkilat şeması olmalı ve bunu bağlı olarak fonksiyonel organizasyon yapısı belirlenmelidir.',
  4
FROM standard
UNION ALL
SELECT 
...
SELECT 
  standard.id,
  'KOS 2.7',
  'Her düzeydeki yöneticileri verilen görevlerin sonucunu izlemeye yönelik mekanizmalar oluşturmalıdır.',
  7
FROM standard;

-- KOS.3 Genel Şartları
WITH standard AS (
  SELECT id FROM ic_standards WHERE code = 'KOS.3'
)
INSERT INTO ic_general_conditions (standard_id, code, condition_text, order_index)
SELECT 
  standard.id,
...
SELECT 
  standard.id,
  'KOS 3.5',
  'Her görev için gerekli eğitim belirlenmeli, bu ihtiyacı giderecek eğitim faaliyetleri her yıl planlanarak yürütülmeli ve gerekliliğinde güncellenmeli',
  5
FROM standard
UNION ALL
SELECT 
  standard.id,
  'KOS 3.6',
  'Personelin yeterliliği ve performansı bağlı olduğu yöneticisi tarafından en az yılda bir kez değerlendirilmeli ve değerlendirme sonuçları personel ile bildirilmelidir.',
  6
FROM standard
UNION ALL
SELECT 
  standard.id,
  'KOS 3.7',
  'Performans değerlendirmesine göre performansı yetersiz bulunan personelin performansını geliştirmeye yönelik önlemler alınmalı, yüksek performans gösteren personelin motivasyonunu sağlayacak mekanizmalar geliştirilmelidir.',
  7
FROM standard;