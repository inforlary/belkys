...
export default function ExecutiveSummary({ selectedYear }: ExecutiveSummaryProps) {
  ...
  const handleExportPDF = () => {
    ...
    const content = `
      <div style="background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); color: white; padding: 14px 18px; border-radius: 4px; margin-bottom: 12px;">
        <h1 style="margin: 0 0 4px 0; font-size: 18px; font-weight: 700;">Yönetici Özet Raporu</h1>
        <p style="margin: 0; font-size: 11px; opacity: 0.9;">Kurum Genel Durumu - ${currentYear}</p>
        <div style="margin-top: 10px; font-size: 32px; font-weight: bold; line-height: 1;">${Math.round(data.overall_progress)}%</div>
        <p style="margin: 2px 0 0 0; font-size: 10px; opacity: 0.9;">Genel İlerleme Oranı</p>
      </div>

      <h2 style="color: #1e40af; border-bottom: 2px solid #2563eb; padding-bottom: 4px; margin: 12px 0 8px 0; font-size: 13px;">Gösterge Durumu</h2>
      <div class="stats-grid">
        <div class="stat-box" style="padding: 10px;">
          <div class="stat-value" style="font-size: 22px;">${data.total_indicators}</div>
          <div class="stat-label" style="font-size: 9px;">Toplam Gösterge</div>
        </div>
        <div class="stat-box" style="border-left: 4px solid #9333ea; padding: 10px;">
          <div class="stat-value" style="color: #9333ea; font-size: 22px;">${data.exceedingTarget}</div>
          <div class="stat-label" style="font-size: 9px;">Hedef Üstü</div>
        </div>
        <div class="stat-box" style="border-left: 4px solid #10b981; padding: 10px;">
          <div class="stat-value" style="color: #10b981; font-size: 22px;">${data.excellent}</div>
          <div class="stat-label" style="font-size: 9px;">Çok İyi</div>
        </div>
        <div class="stat-box" style="border-left: 4px solid #22c55e; padding: 10px;">
          <div class="stat-value" style="color: #22c55e; font-size: 22px;">${data.good}</div>
          <div class="stat-label" style="font-size: 9px;">İyi</div>
        </div>
      </div>

      <div class="stats-grid" style="margin-top: 6px;">
        <div class="stat-box" style="border-left: 4px solid #eab308; padding: 10px;">
          <div class="stat-value" style="color: #ca8a04; font-size: 22px;">${data.moderate}</div>
          <div class="stat-label" style="font-size: 9px;">Orta</div>
        </div>
        <div class="stat-box" style="border-left: 4px solid #f97316; padding: 10px;">
          <div class="stat-value" style="color: #f97316; font-size: 22px;">${data.weak}</div>
          <div class="stat-label" style="font-size: 9px;">Zayıf</div>
        </div>
        <div class="stat-box" style="border-left: 4px solid #dc2626; padding: 10px;">
          <div class="stat-value" style="color: #dc2626; font-size: 22px;">${data.veryWeak}</div>
          <div class="stat-label" style="font-size: 9px;">Çok Zayıf</div>
        </div>
      </div>

      <h2 style="color: #1e40af; border-bottom: 2px solid #2563eb; padding-bottom: 4px; margin: 12px 0 8px 0; font-size: 13px;">Operasyonel Metrikler</h2>
      <div class="stats-grid">
        <div class="stat-box" style="border-left: 4px solid ${data.data_completion >= 75 ? '#10b981' : '#eab308'}; padding: 10px;">
          <div class="stat-value" style="color: ${data.data_completion >= 75 ? '#10b981' : '#ca8a04'}; font-size: 22px;">${Math.round(data.data_completion)}%</div>
          <div class="stat-label" style="font-size: 9px;">Veri Giriş Oranı</div>
        </div>
        <div class="stat-box" style="border-left: 4px solid #eab308; padding: 10px;">
          <div class="stat-value" style="color: #ca8a04; font-size: 22px;">${data.pending_approvals}</div>
          <div class="stat-label" style="font-size: 9px;">Bekleyen Onay</div>
        </div>
        <div class="stat-box" style="border-left: 4px solid #dc2626; padding: 10px;">
          <div class="stat-value" style="color: #dc2626; font-size: 22px;">${data.overdue_activities}</div>
          <div class="stat-label" style="font-size: 9px;">Gecikmiş Faaliyet</div>
        </div>
      </div>

      ${data.strategic_plans.length > 0 ? `
        <h2 style="color: #1e40af; border-bottom: 2px solid #2563eb; padding-bottom: 4px; margin: 14px 0 8px 0; font-size: 13px;">Stratejik Planlar</h2>
        ${generateTableHTML(plansHeaders, plansRows)}
      ` : ''}

      ${data.top_performers.length > 0 ? `
        <h2 style="color: #1e40af; border-bottom: 2px solid #2563eb; padding-bottom: 4px; margin: 14px 0 6px 0; font-size: 13px;">Hedef Sapması Gösteren Göstergeler</h2>
        <p style="margin-bottom: 8px; color: #64748b; font-size: 10px;">Hedefin üzerinde performans gösteren göstergeler</p>
        ${generateTableHTML(topPerformersHeaders, topPerformersRows)}
      ` : ''}

      ${data.concerns.length > 0 ? `
        <h2 style="color: #1e40af; border-bottom: 2px solid #2563eb; padding-bottom: 4px; margin: 14px 0 6px 0; font-size: 13px;">Geride Kalan Göstergeler</h2>
        <p style="margin-bottom: 8px; color: #64748b; font-size: 10px;">Hedefin gerisinde kalan ve dikkat gerektiren göstergeler</p>
        ${generateTableHTML(concernsHeaders, concernsRows)}
      ` : ''}

      <h2 style="color: #1e40af; border-bottom: 2px solid #2563eb; padding-bottom: 4px; margin: 14px 0 8px 0; font-size: 13px;">Öneriler ve Dikkat Noktaları</h2>
      ${generateTableHTML(recommendationsHeaders, recommendationsRows)}

      <div style="margin-top: 14px; padding: 10px 14px; background: #f1f5f9; border-radius: 4px; border-left: 3px solid #2563eb;">
        ...
        <p style="margin: 3px 0 0 0; color: #475569; font-size: 9px;">
