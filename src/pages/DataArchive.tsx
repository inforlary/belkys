...
export default function DataArchive() {
  ...
  const getIndicatorTarget = (indicatorId: string, indicator?: Indicator) => {
    const target = indicatorTargets.find(
      t => t.indicator_id === indicatorId
    );
    if (target?.target_value) return target.target_value;
    return indicator?.target_value || null;
  };
